// src/components/ui/AddBookIco.jsx - FIXED VERSION
import React from "react";
import { useNavigate } from "react-router-dom";
import styled from "styled-components";
import { UseAuth } from "../../context/AuthContext";

const AddBookIco = () => {
  const navigate = useNavigate();
  const { user, isAuthenticated } = UseAuth();

  const handleAddBookClick = (e) => {
    // Ngăn mọi event bubbling
    e.preventDefault();
    e.stopPropagation();
    
    console.log("🎯 Button clicked - Starting navigation");
    console.log("User:", user);
    console.log("Is Authenticated:", isAuthenticated);
    
    if (!isAuthenticated) {
      console.log("➡️ Redirecting to login");
      navigate("/login");
      return;
    }

    if (user?.role !== "admin") {
      console.log("❌ User is not admin");
      alert("Only admins can add books");
      return;
    }

    console.log("✅ Navigating to /add-book");
    
    // Navigation với state để track
    navigate("/add-book", { 
      replace: false,
      state: { from: "addbook-button" }
    });
  };

  // Chỉ hiển thị nút cho admin
  if (!isAuthenticated || user?.role !== "admin") {
    return null;
  }

  return (
    <StyledWrapper 
      onClick={(e) => {
        e.stopPropagation();
      }}
      onMouseDown={(e) => {
        e.stopPropagation();
      }}
    >
      <button 
        className="button" 
        onClick={handleAddBookClick}
        type="button"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="1.25rem"
          height="1.25rem"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={2}
        >
          <path d="M12 19v-7m0 0V5m0 7H5m7 0h7" />
        </svg>
        Create Book
      </button>
    </StyledWrapper>
  );
};

const StyledWrapper = styled.div`
  /* Đảm bảo wrapper không block events */
  position: relative;
  z-index: 10;
  
  .button {
    cursor: pointer !important; /* Override CSS global cursor */
    font-size: 1rem;
    line-height: 1.5rem;
    padding: 0.625rem 1rem;
    color: rgb(242 242 242);
    background-color: rgb(79 70 229);
    background: linear-gradient(144deg, #af40ff, #5b42f3 50%, #00ddeb);
    font-weight: 600;
    border-radius: 0.5rem;
    border-style: none;
    display: flex;
    justify-content: center;
    align-items: center;
    gap: 0.5rem;
    transition: all 0.35s linear;
    
    /* Đảm bảo button có thể click */
    pointer-events: auto;
    user-select: none;
  }

  .button:hover {
    box-shadow: inset 0 5px 25px 0 #af40ff, inset 0 10px 15px 0px #5b42f3,
      inset 0 5px 25px 0px #00ddeb;
    transform: translateY(-2px);
  }
  
  .button:active {
    transform: translateY(0);
  }
`;

export default AddBookIco;